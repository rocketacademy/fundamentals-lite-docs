...
Block scoping applies to any variable defined in a block of code within a pair of `{}`. In Coding Fundamentals, this is mostly blocks of code defined by if-statements.
...
For the following exercises, start with the Coding Fundamentals Starter Code template and replace the code in `script.js` with the following dice game code.
