...
However, the **concept** of the array is for holding data of the same kind, e.g. numerical dice rolls, and this almost always means the data is of the same JavaScript data type. Please do not deviate from this in Coding Fundamentals.
{% endhint %}
...
There are 2 methods we will use often in Coding Fundamentals to add and remove elements from arrays: `push` and `pop`.
